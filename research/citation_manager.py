...
class ComparisonEngine:
    ...
    def _find_winner(
        ...
        if len(
            ...
            if lower_is_better:
                return min(
                    numeric_values,
                    key=lambda label: numeric_values[label],
                )
    ...
    def generate_summary(
        ...
        best = max(
            winner_counts,
            key=lambda label: winner_counts[label],
        )
...
class CitationManager:
    """Manage structured research citations without external lookups."""
    ...
    def add_source(
        ...
        *,
        ...
        citation = {
            "id": f"citation_{len(self._citations) + 1}",
            "title": title,
            "url": url,
            "author": author,
    ...
    def format_citation(
        ...
        author = str(citation.get("author", "")).strip()
        url = str(citation.get("url", "")).strip()
        published_at = str(citation.get("published_at", "")).strip()
        accessed_at = str(citation.get("accessed_at", "")).strip()

        parts = []
        if author:
            parts.append(author)
        if title:
            parts.append(f'"{title}"' if style.lower() == "mla" else title)
        if published_at:
            parts.append(published_at)
        if url:
            parts.append(url)
        if accessed_at:
            parts.append(f"accessed {accessed_at}")

        return ". ".join(parts) + ("." if parts else "")

    def bibliography(self, style: str = "plain") -> list[str]:
        return [
            self.format_citation(citation, style=style)
